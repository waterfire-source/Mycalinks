...
/**
 * Page object for the cart table in the sales page
 */
export class SaleCartTable {
  readonly page: Page;
  ...
  constructor(page: Page) {
    ...
    // Table container - using the table that contains "商品画像" text
    this.tableContainer = page
      .locator('table')
      .filter({ hasText: '商品画像' })
      .first();

    // Table rows - each row has a unique key in format "${cart.productId}-${rowIndex}"
    ...
    this.unitPriceInput = this.tableRows
      ...
      .locator('input')
      .first();
    this.discountButton = this.tableRows.locator('td').nth(2).getByText('割引');
