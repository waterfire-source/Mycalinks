...
const SearchField: React.FC<Props> = ({
  ...
  const [expansion, setExpansion] = useState(searchState.expansion || '');
  const [cardnumber, setCardnumber] = useState(searchState.cardnumber || '');
    ...
    >
      <Stack direction="row" alignItems="center" gap="10px" sx={{ ...sx }}>
        ...
        <TextField
          ...
          type="text"
          size="small"
          onChange={(e) => setCardnumber(toHalfWidthOnly(e.target.value))}
          onKeyDown={(e) => {
            if (e.key === 'Enter') {
              setSearchState((prev) => ({
                ...prev,
