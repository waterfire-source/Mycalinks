...
export const CreateItemModal = ({
  ...
  const nonCardCategory = useMemo(
    () =>
      category?.itemCategories.filter(
        (c) => c.handle !== ItemCategoryHandle.CARD,
      ),
    [category],
  );
    ...
    <Grid container spacing={4}>
      ...
      <Grid item xs={3}>
        <Stack gap={2} width="fit-content" ml="auto">
          ...
          <SecondaryButtonWithIcon
            onClick={() => {
              handleChangeModalState(ModalType.SelectOneselfItems);
            }}
            sx={{ textTransform: 'none' }}
          >
            既存の独自商品を選択
