    ...
    switch (type) {
      case 'payment.updated': {
        ...
        if (
          ...
          const createPaymentResult = await API.primaryDb.payment.upsert({
            where: {
              transaction_id,
            },
            update: {},
          ...
          switch (targetObject.source_type) {
            ...
            case 'CARD': {
              ...
              await API.primaryDb.payment.update({
                where: {
          ...
          //ステータスを更新する
          await API.primaryDb.transaction.update({
            where: {
              id: transaction_id,
            },
            data: {
      ...
      case 'refund.updated': {
        ...
        if (
          ...
          const paymentInfo = await API.primaryDb.payment.findFirst({
            where: {
          ...
          transaction_id =
            paymentInfo?.transaction.return_transactions[0]?.id || 0;
          if (transaction_id) {
            await API.primaryDb.transaction.update({
              where: {
                id: transaction_id,
      ...
      case 'terminal.checkout.updated': {
        ...
        if (
          ...
          const storeInfo = await API.db.store.findUnique({
            where: {
              id: transactionService.core.targetObject?.store_id,
            },
            include: {
              ec_setting: true,
              accounts: true,
          ...
          transactionService.setIds({
            storeId: storeInfo.id,
          });
          transactionService.setResources({
            store: storeInfo,
          });
