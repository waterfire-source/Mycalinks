...
import { ApiResponse, getEcOrderStatsByConditionApi } from 'api-generator';
import { customDayjs } from 'common';

export const GET = BackendAPI.create(
  getEcOrderStatsByConditionApi,
  async (API, { params, query }) => {
    ...
    const selectRes = await API.db.$queryRaw<
      ApiResponse<typeof getEcOrderStatsByConditionApi>['summaryByConditions']
    >`
    SELECT
      COALESCE(feop.condition_option_display_name, '未設定')        AS condition_display_name,
    ...
    AND feop.condition_option_display_name != ''
    AND TRIM(feop.condition_option_display_name) != ''
    ${whereInput}
    GROUP BY feop.condition_option_id;
    `;

    const summaryByConditions = selectRes.map((res) => ({
      condition_display_name: res.condition_display_name,
