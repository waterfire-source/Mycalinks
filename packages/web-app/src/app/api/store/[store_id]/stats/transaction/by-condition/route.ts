...
import { ApiResponse, getTransactionStatsByConditionApi } from 'api-generator';
import { customDayjs } from 'common';

export const GET = BackendAPI.create(
  getTransactionStatsByConditionApi,
  async (API, { params, query }) => {
    ...
    const selectRes = await API.db.$queryRaw<
      ApiResponse<
        typeof getTransactionStatsByConditionApi
      >['summaryByConditions']
    >`
    SELECT
      COALESCE(ftp.condition_option_display_name, '未設定')     AS condition_display_name,
    ...
    AND ftp.condition_option_display_name != ''
    AND TRIM(ftp.condition_option_display_name) != ''
    ${whereInput}
    GROUP BY ftp.condition_option_id;
    `;
