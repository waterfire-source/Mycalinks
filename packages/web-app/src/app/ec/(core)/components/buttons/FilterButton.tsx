...
// - storeIds: string[] - 選択された店舗ID (カンマ区切り)
...
export const FilterButton = () => {
  ...
  const toggleDrawer = (newOpen: boolean) => () => {
    if (!newOpen && open) {
      // ドロワーを閉じる時に、検索ボタンが押されていなければ状態をリセット
      resetToLastSearchState();
    }
    setOpen(newOpen);
  };
  ...
  const resetToLastSearchState = () => {
    if (lastSearchState) {
      ...
      setSelectedStoreIds(lastSearchState.storeIds || []);
      setSelectedSpecialties(lastSearchState.specialties || []);
      setSelectedBoxTypes(lastSearchState.boxTypes || []);
    } else {
      // 最後の検索状態がない場合はすべてクリア
      handleClearAll();
    }
        ...
        >
          <Typography
            ...
            sx={{
              ...
              display: 'flex',
              alignItems: 'center',
              color: isEcLoading ? 'grey.300' : '#000',
              marginRight: activeFilterCount > 0 ? '15px' : '0px',
            }}
          >
            絞り込み
          </Typography>
          ...
          <Box sx={{ flex: 1, overflow: 'auto', py: 1 }}>
                ...
                {cardCondition.map((condition) => (
                  <FormControlLabel
                    ...
                    control={
                      <Checkbox
                        size="small"
                        checked={selectedCardConditions.includes(
                          condition.value,
                        )}
                        onChange={() =>
                          handleCardConditionChange(condition.value)
                        }
                      />
                    }
                    label={
                    ...
                    <FormControlLabel
                      ...
                      control={
                        <Checkbox
                          ...
                          onChange={() =>
                            handleStoreChange(store.id.toString())
                          }
                        />
                      }
                      label={
                    ...
                    <FormControlLabel
                      ...
                      control={
                        <Checkbox
                          ...
                          onChange={() =>
                            handleSpecialtyChange(condition.value)
                          }
                        />
                      }
                      label={
                ...
                {boxCondition.map((condition) => (
                  <FormControlLabel
                    ...
                    control={
                      <Checkbox
                        size="small"
                        checked={selectedBoxConditions.includes(
                          condition.value,
                        )}
                        onChange={() =>
                          handleBoxConditionChange(condition.value)
                        }
                      />
                    }
                    label={
            ...
            {itemOptions &&
              itemOptions.map((option, optionIndex) => (
                <Box key={optionIndex} sx={{ mb: 3, mx: 2 }}>
                  <Divider sx={{ my: 2 }} />
