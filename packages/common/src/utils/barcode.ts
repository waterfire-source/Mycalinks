...
export class Barcode {
  ...
  public static verifyBarcode(token: string): {
    ...
    customerId?: number; //POSの顧客ID
    corporationId?: number; //従業員バーコードの場合 [TODO] returnわかりやすくしたい
  } {
    if (token.length != 24 && token.length != 12)
      throw new Error('会員コードが不適切です');

    let barcodeKind: barcodeKind;

    let userId: number | undefined = undefined;
    let customerId: number | undefined = undefined;
    const corporationId: number | undefined = undefined;
  ...
  public static generateCustomerBarcode(customerId: number): string {
    let tokenString = '5';
