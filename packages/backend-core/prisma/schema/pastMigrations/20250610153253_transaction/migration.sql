/*
  ...
  - You are about to alter the column `payment_info` on the `Ec_Order` table. The data in that column could be lost. The data in that column will be cast from `LongText` to `Json`.
  - You are about to alter the column `source_event_json` on the `Ec_Order_Payment` table. The data in that column could be lost. The data in that column will be cast from `LongText` to `Json`.
  - You are about to alter the column `payment_info_json` on the `Ec_Order_Payment` table. The data in that column could be lost. The data in that column will be cast from `LongText` to `Json`.
  - You are about to alter the column `source_event_json` on the `Payment` table. The data in that column could be lost. The data in that column will be cast from `LongText` to `Json`.
  - You are about to drop the column `deposit_transaction_id` on the `Reservation_Reception_Product` table. All the data in the column will be lost.
  - You are about to drop the column `received_transaction_id` on the `Reservation_Reception_Product` table. All the data in the column will be lost.

*/
-- DropForeignKey
ALTER TABLE `Account` DROP FOREIGN KEY `Account_group_id_fkey`;

-- DropForeignKey
ALTER TABLE `Reservation_Reception_Product` DROP FOREIGN KEY `Reservation_Reception_Product_deposit_transaction_id_fkey`;

-- DropForeignKey
ALTER TABLE `Reservation_Reception_Product` DROP FOREIGN KEY `Reservation_Reception_Product_received_transaction_id_fkey`;

-- DropForeignKey
ALTER TABLE `Transaction` DROP FOREIGN KEY `Transaction_original_transaction_id_fkey`;

-- DropIndex
DROP INDEX `Account_group_id_fkey` ON `Account`;

-- DropIndex
DROP INDEX `Reservation_Reception_Product_deposit_transaction_id_reserva_key` ON `Reservation_Reception_Product`;

-- DropIndex
DROP INDEX `Reservation_Reception_Product_received_transaction_id_reserv_key` ON `Reservation_Reception_Product`;

-- DropIndex
DROP INDEX `Transaction_original_transaction_id_fkey` ON `Transaction`;
...
-- AlterTable
ALTER TABLE `Ec_Order_Payment` MODIFY `source_event_json` JSON NULL,
    MODIFY `payment_info_json` JSON NULL;
...
-- AlterTable
ALTER TABLE `Payment` MODIFY `source_event_json` JSON NULL,
    MODIFY `bank__checked` TINYINT UNSIGNED NOT NULL DEFAULT false;
...
-- AlterTable
ALTER TABLE `Reservation_Reception_Product` DROP COLUMN `deposit_transaction_id`,
    DROP COLUMN `received_transaction_id`,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();
...
-- AlterTable
ALTER TABLE `Transaction_Cart` ADD COLUMN `reservation_reception_product_id_for_deposit` INTEGER UNSIGNED NULL,
    ADD COLUMN `reservation_reception_product_id_for_receive` INTEGER UNSIGNED NULL;
