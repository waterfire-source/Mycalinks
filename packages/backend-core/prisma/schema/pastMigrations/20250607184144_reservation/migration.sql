/*
  ...
  - The values [DELETED] on the enum `Reservation_status` will be removed. If these variants are still used in the database, this will fail.
...
ALTER TABLE `Reservation` MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    ...
    MODIFY `status` ENUM('NOT_STARTED', 'OPEN', 'CLOSED', 'FINISHED') NOT NULL DEFAULT 'NOT_STARTED';
