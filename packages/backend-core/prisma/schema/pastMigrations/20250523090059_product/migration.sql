...
-- AlterTable
ALTER TABLE `Product` ADD COLUMN `ec_pos_stock_gap` INTEGER GENERATED ALWAYS AS (stock_number - ec_stock_number) STORED,
    MODIFY `is_buy_only` TINYINT UNSIGNED NOT NULL DEFAULT false,
...
-- AlterTable
ALTER TABLE `Product` 
    ADD COLUMN `specific_ec_publishable_stock_number` INTEGER GENERATED ALWAYS AS (GREATEST(0, ec_pos_stock_gap - pos_reserved_stock_number)) VIRTUAL;
...
-- CreateTable
CREATE TABLE `Outbox_Product_Stock_History` (
    `product_stock_history_id` INTEGER UNSIGNED NOT NULL,
    `product_id` INTEGER UNSIGNED NOT NULL,
    `store_id` INTEGER UNSIGNED NOT NULL,
...
-- CreateIndex
CREATE INDEX `Product_ec_pos_stock_gap_idx` ON `Product`(`ec_pos_stock_gap`);
