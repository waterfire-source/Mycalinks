/*
  ...
  - The primary key for the `Product_Price` table will be changed. If it partially fails, the table could be left without primary key constraint.
...
-- AlterTable
ALTER TABLE `Outbox_Product` ADD COLUMN `ochanoko_product_id` INTEGER UNSIGNED NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP();
...
ALTER TABLE `Product_Price` DROP PRIMARY KEY,
    MODIFY `kind` ENUM('buy', 'sell', 'ec') NOT NULL,
    MODIFY `date` DATE NOT NULL DEFAULT (CURRENT_DATE()),
    ADD PRIMARY KEY (`product_id`, `kind`, `date`);
