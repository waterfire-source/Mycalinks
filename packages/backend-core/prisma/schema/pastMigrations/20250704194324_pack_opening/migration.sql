...
-- AlterTable
ALTER TABLE `Product_Stock_History` MODIFY `source_kind` ENUM('product', 'transaction_sell', 'transaction_buy', 'transaction_sell_return', 'transaction_buy_return', 'stocking', 'stocking_rollback', 'loss', 'bundle', 'bundle_release', 'original_pack', 'original_pack_release', 'pack_opening', 'pack_opening_unregister', 'pack_opening_rollback', 'pack_opening_unregister_rollback', 'appraisal_create', 'appraisal_return', 'transfer', 'ec_sell', 'ec_sell_return', 'consignment_create') NOT NULL,
...
ALTER TABLE `Product_Wholesale_Price_History` MODIFY `arrived_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `resource_type` ENUM('PRODUCT', 'BUNDLE', 'ORIGINAL_PACK', 'TRANSACTION', 'STOCKING', 'PACK_OPENING', 'PACK_OPENING_UNREGISTER', 'EC_ORDER', 'LOSS', 'APPRAISAL', 'CHILD') NOT NULL DEFAULT 'PRODUCT',
