...
CREATE TABLE `Myca_Item` (
    ...
    `market_price_gap_rate` INTEGER GENERATED ALWAYS AS (CASE WHEN previous_market_price IS NULL THEN NULL WHEN previous_market_price = 0 THEN 100 ELSE (market_price - previous_market_price) / previous_market_price * 100 END) VIRTUAL,
...
-- 外部キーを追加する前にやっちゃう
INSERT INTO Myca_Item(myca_item_id,market_price,market_price_updated_at,previous_market_price)
SELECT myca_item_id,market_price,market_price_updated_at,previous_market_price
FROM Item
WHERE myca_item_id IS NOT NULL
GROUP BY myca_item_id;
