...
ALTER TABLE `Product_Wholesale_Price_History` MODIFY `arrived_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `resource_type` ENUM('PRODUCT', 'BUNDLE', 'ORIGINAL_PACK', 'TRANSACTION', 'STOCKING', 'EC_ORDER', 'LOSS', 'APPRAISAL', 'CHILD') NOT NULL DEFAULT 'PRODUCT',
...
ALTER TABLE `Stocking` MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `status` ENUM('NOT_YET', 'FINISHED', 'CANCELED', 'ROLLBACK') NOT NULL DEFAULT 'NOT_YET';
