/*
  ...
  - A unique constraint covering the columns `[carton_item_id]` on the table `Item` will be added. If there are existing duplicate values, this will fail.
...
ALTER TABLE `Announcement` DROP COLUMN `content`,
    ADD COLUMN `publish_at` DATE NULL,
    ADD COLUMN `status` ENUM('UNPUBLISHED', 'PUBLISHED', 'DELETED') NOT NULL DEFAULT 'UNPUBLISHED',
...
-- AlterTable
ALTER TABLE `Item` ADD COLUMN `carton_item_id` INTEGER UNSIGNED NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
...
-- CreateIndex
CREATE UNIQUE INDEX `Item_carton_item_id_key` ON `Item`(`carton_item_id`);
