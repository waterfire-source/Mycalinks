-- DropIndex
DROP INDEX `Item_search_keyword_rarity_expansion_idx` ON `Item`;

-- DropIndex
DROP INDEX `Product_display_name_idx` ON `Product`;
...
-- AlterTable
ALTER TABLE `Item_Category` ADD COLUMN `order_number` INTEGER NOT NULL DEFAULT 0,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
    MODIFY `deleted` TINYINT UNSIGNED NOT NULL DEFAULT false,
    MODIFY `hidden` TINYINT UNSIGNED NOT NULL DEFAULT false,
...
-- CreateIndex
CREATE INDEX `Product_store_id_deleted_stock_number_is_active_idx` ON `Product`(`store_id`, `deleted`, `stock_number`, `is_active`);
