/*
  ...
  - You are about to drop the column `unregister_product_loss_id` on the `Pack_Open_History` table. All the data in the column will be lost.

*/
-- DropForeignKey
ALTER TABLE `Pack_Open_History` DROP FOREIGN KEY `Pack_Open_History_unregister_product_loss_id_fkey`;

-- DropIndex
DROP INDEX `Pack_Open_History_unregister_product_loss_id_fkey` ON `Pack_Open_History`;
...
-- AlterTable
ALTER TABLE `Pack_Open_History` DROP COLUMN `unregister_product_loss_id`,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `status` ENUM('DRAFT', 'FINISHED', 'ROLLBACK') NOT NULL DEFAULT 'FINISHED',
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();
