/*
  ...
  - A unique constraint covering the columns `[deposit_transaction_id,reservation_id]` on the table `Reservation_Reception_Product` will be added. If there are existing duplicate values, this will fail.
  - A unique constraint covering the columns `[received_transaction_id,reservation_id]` on the table `Reservation_Reception_Product` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `Ec_Order` ALTER COLUMN `code` DROP DEFAULT,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();

-- AlterTable
ALTER TABLE `Ec_Order_Cart_Store` ALTER COLUMN `code` DROP DEFAULT,
...
-- CreateIndex
CREATE UNIQUE INDEX `Reservation_Reception_Product_deposit_transaction_id_reserva_key` ON `Reservation_Reception_Product`(`deposit_transaction_id`, `reservation_id`);

-- CreateIndex
CREATE UNIQUE INDEX `Reservation_Reception_Product_received_transaction_id_reserv_key` ON `Reservation_Reception_Product`(`received_transaction_id`, `reservation_id`);
