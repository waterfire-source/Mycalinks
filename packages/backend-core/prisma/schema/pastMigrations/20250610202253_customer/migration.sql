/*
  ...
  - You are about to drop the column `id` on the `Dim_Transaction_Customer` table. All the data in the column will be lost.
  - Added the required column `customer_id` to the `Dim_Transaction_Customer` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `Dim_Transaction_Customer` DROP COLUMN `id`,
    ADD COLUMN `customer_id` INTEGER UNSIGNED NOT NULL;
...
-- CreateIndex
CREATE INDEX `Dim_Transaction_Customer_transaction_id_customer_id_idx` ON `Dim_Transaction_Customer`(`transaction_id`, `customer_id`);
