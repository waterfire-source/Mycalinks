...
-- AlterTable
ALTER TABLE `Pack_Open_History` ADD COLUMN `unregister_product_loss_id` INTEGER UNSIGNED NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();
...
-- AddForeignKey
ALTER TABLE `Pack_Open_History` ADD CONSTRAINT `Pack_Open_History_unregister_product_loss_id_fkey` FOREIGN KEY (`unregister_product_loss_id`) REFERENCES `Loss`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
