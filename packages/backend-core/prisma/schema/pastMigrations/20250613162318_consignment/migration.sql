/*
  ...
  - You are about to drop the column `type` on the `Product_Stock_History` table. All the data in the column will be lost.
...
-- AlterTable
ALTER TABLE `Product` ADD COLUMN `consignment_client_id` INTEGER UNSIGNED NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
...
-- AlterTable
ALTER TABLE `Product_Stock_History` DROP COLUMN `type`,
    MODIFY `source_kind` ENUM('product', 'transaction_sell', 'transaction_buy', 'transaction_sell_return', 'transaction_buy_return', 'stocking', 'loss', 'bundle', 'bundle_release', 'original_pack', 'original_pack_release', 'pack_opening', 'pack_opening_unregister', 'appraisal_create', 'appraisal_return', 'transfer', 'ec_sell', 'ec_sell_return', 'consignment_create') NOT NULL,
...
-- AlterTable
ALTER TABLE `Transaction_Cart` ADD COLUMN `consignment_commission_price` INTEGER NOT NULL DEFAULT 0;
