/*
  ...
  - You are about to drop the column `previous_transaction_id` on the `Transaction` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[original_transaction_id]` on the table `Transaction` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `customer_id` to the `Reservation_Reception_Product` table without a default value. This is not possible if the table is not empty.

*/
-- DropForeignKey
ALTER TABLE `Transaction` DROP FOREIGN KEY `Transaction_previous_transaction_id_fkey`;

-- DropIndex
DROP INDEX `Transaction_previous_transaction_id_key` ON `Transaction`;
...
-- AlterTable
ALTER TABLE `Reservation_Reception_Product` ADD COLUMN `customer_id` INTEGER UNSIGNED NOT NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();
...
ALTER TABLE `Transaction` 
    ADD COLUMN `original_transaction_id` INTEGER UNSIGNED NULL,
...
UPDATE Transaction SET original_transaction_id = previous_transaction_id;

-- DropForeignKey
ALTER TABLE `Transaction` DROP COLUMN `previous_transaction_id`;

-- CreateIndex
CREATE UNIQUE INDEX `Transaction_original_transaction_id_key` ON `Transaction`(`original_transaction_id`);
