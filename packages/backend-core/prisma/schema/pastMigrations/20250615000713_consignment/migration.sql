/*
  ...
  - You are about to drop the column `consignment_commission_price` on the `Transaction_Cart` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[store_id,full_name]` on the table `Consignment_Client` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `Consignment_Client` ADD COLUMN `deleted` TINYINT UNSIGNED NOT NULL DEFAULT false,
    MODIFY `enabled` TINYINT UNSIGNED NOT NULL DEFAULT true,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();
...
-- AlterTable
ALTER TABLE `Item_Category_Condition_Option` ADD COLUMN `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `deleted` TINYINT UNSIGNED NOT NULL DEFAULT false;
...
-- AlterTable
ALTER TABLE `Transaction` ADD COLUMN `total_consignment_sale_price` INTEGER NOT NULL DEFAULT 0,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
...
-- AlterTable
ALTER TABLE `Transaction_Cart` DROP COLUMN `consignment_commission_price`,
    ADD COLUMN `consignment_commission_unit_price` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `consignment_sale_unit_price` INTEGER NOT NULL DEFAULT 0;

-- CreateIndex
CREATE UNIQUE INDEX `Consignment_Client_store_id_full_name_key` ON `Consignment_Client`(`store_id`, `full_name`);
