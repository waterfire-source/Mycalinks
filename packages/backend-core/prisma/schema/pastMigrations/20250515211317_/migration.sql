/*
  ...
  - You are about to drop the column `store_id` on the `Appraisal_Option` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[appraisal_company_id,display_name]` on the table `Appraisal_Option` will be added. If there are existing duplicate values, this will fail.

*/
-- DropIndex
DROP INDEX `Appraisal_Option_store_id_appraisal_company_id_display_name_key` ON `Appraisal_Option`;
...
-- AlterTable
ALTER TABLE `Appraisal_Option` DROP COLUMN `store_id`,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP();
...
-- CreateIndex
CREATE UNIQUE INDEX `Appraisal_Option_appraisal_company_id_display_name_key` ON `Appraisal_Option`(`appraisal_company_id`, `display_name`);
