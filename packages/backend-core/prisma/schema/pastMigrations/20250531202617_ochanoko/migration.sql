/*
  ...
  - A unique constraint covering the columns `[store_id,ochanoko_product_id]` on the table `Product` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `Ec_Order` ADD COLUMN `payment_method_display_name` VARCHAR(191) NULL,
    MODIFY `code` VARCHAR(32) NOT NULL DEFAULT concat(lpad(hex(unix_timestamp(now(3)) * 1000), 12, '0'),'7',substr(hex(random_bytes(2)), 2),hex(floor(rand() * 4 + 8)),substr(hex(random_bytes(8)), 2)),
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();
...
-- AlterTable
ALTER TABLE `Ec_Order_Payment` MODIFY `service` ENUM('GMO', 'ORIGINAL', 'OCHANOKO', 'SHOPIFY') NOT NULL;

-- AlterTable
ALTER TABLE `Ec_Setting` ADD COLUMN `ochanoko_account_id` VARCHAR(191) NULL,
    ADD COLUMN `ochanoko_api_token` TEXT NULL,
    ADD COLUMN `ochanoko_email` VARCHAR(191) NULL,
    ADD COLUMN `ochanoko_password` VARCHAR(191) NULL,
...
-- AlterTable
ALTER TABLE `Product` ADD COLUMN `ochanoko_product_id` INTEGER UNSIGNED NULL,
    MODIFY `is_buy_only` TINYINT UNSIGNED NOT NULL DEFAULT false,
...
-- AlterTable
ALTER TABLE `Store` ADD COLUMN `ochanoko_ec_enabled` TINYINT UNSIGNED NOT NULL DEFAULT false,
    MODIFY `code` VARCHAR(32) NOT NULL DEFAULT concat(lpad(hex(unix_timestamp(now(3)) * 1000), 12, '0'),'7',substr(hex(random_bytes(2)), 2),hex(floor(rand() * 4 + 8)),substr(hex(random_bytes(8)), 2)),
...
-- CreateIndex
CREATE UNIQUE INDEX `Product_store_id_ochanoko_product_id_key` ON `Product`(`store_id`, `ochanoko_product_id`);
