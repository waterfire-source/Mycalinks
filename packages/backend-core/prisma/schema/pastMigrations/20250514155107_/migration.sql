...
-- DropIndex
DROP INDEX `Product_product_code_idx` ON `Product`;

-- DropIndex
DROP INDEX `Product_store_id_product_code_key` ON `Product`;
...
-- AlterTable
ALTER TABLE `Item` ADD COLUMN `search_keyword` VARCHAR(500) GENERATED ALWAYS AS (
    CONCAT(
        IFNULL(`display_name`, ''),
...
-- AlterTable
ALTER TABLE `Product` DROP COLUMN `product_code`,
    MODIFY `is_buy_only` TINYINT UNSIGNED NOT NULL DEFAULT false,
