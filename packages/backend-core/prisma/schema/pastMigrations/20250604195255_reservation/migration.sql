/*
  ...
  - Added the required column `deposit_price` to the `Reservation` table without a default value. This is not possible if the table is not empty.
  - Added the required column `end_at` to the `Reservation` table without a default value. This is not possible if the table is not empty.
  - Added the required column `limit_count` to the `Reservation` table without a default value. This is not possible if the table is not empty.
  - Added the required column `limit_count_per_user` to the `Reservation` table without a default value. This is not possible if the table is not empty.
  - Added the required column `remaining_price` to the `Reservation` table without a default value. This is not possible if the table is not empty.
  - Added the required column `start_at` to the `Reservation` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `Reservation` ADD COLUMN `deposit_price` INTEGER NOT NULL,
    ADD COLUMN `description` VARCHAR(300) NULL,
    ADD COLUMN `end_at` TIMESTAMP(0) NOT NULL,
    ADD COLUMN `limit_count` INTEGER NOT NULL,
    ADD COLUMN `limit_count_per_user` INTEGER NOT NULL,
    ADD COLUMN `remaining_price` INTEGER NOT NULL,
    ADD COLUMN `start_at` TIMESTAMP(0) NOT NULL,
    ADD COLUMN `status` ENUM('NOT_STARTED', 'ONGOING', 'FINISHED', 'DELETED') NOT NULL DEFAULT 'NOT_STARTED',
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();
...
CREATE TABLE `Reservation_Reception_Product` (
    ...
    `deposit_item_count` INTEGER NOT NULL DEFAULT 0,
    ...
    PRIMARY KEY (`reservation_reception_id`, `reservation_id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
