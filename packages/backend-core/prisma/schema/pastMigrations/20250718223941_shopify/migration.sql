/*
  ...
  - You are about to drop the column `shopify_access_token` on the `Store` table. All the data in the column will be lost.
  - You are about to drop the column `shopify_shop_domain` on the `Store` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[shopify_shop_domain]` on the table `Ec_Setting` will be added. If there are existing duplicate values, this will fail.

*/
-- DropIndex
DROP INDEX `Store_shopify_shop_domain_key` ON `Store`;
...
-- AlterTable
ALTER TABLE `Ec_Setting` ADD COLUMN `shopify_access_token` VARCHAR(191) NULL,
    ADD COLUMN `shopify_shop_domain` VARCHAR(191) NULL,
...
-- AlterTable
ALTER TABLE `Store` DROP COLUMN `shopify_access_token`,
    DROP COLUMN `shopify_shop_domain`,
    ADD COLUMN `shopify_ec_enabled` TINYINT UNSIGNED NOT NULL DEFAULT false,
...
-- CreateIndex
CREATE UNIQUE INDEX `Ec_Setting_shopify_shop_domain_key` ON `Ec_Setting`(`shopify_shop_domain`);
