...
-- AlterTable
ALTER TABLE `Ec_Order_Payment` ADD COLUMN `payment_info_json` JSON NULL,
    ADD COLUMN `status` ENUM('PAYING', 'COMPLETED', 'CANCELED') NOT NULL DEFAULT 'PAYING';
...
-- CreateIndex
CREATE INDEX `Ec_Order_Payment_mode_service_status_source_id_order_id_idx` ON `Ec_Order_Payment`(`mode`, `service`, `status`, `source_id`, `order_id`);
