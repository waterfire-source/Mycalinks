/*
  ...
  - A unique constraint covering the columns `[shopify_shop_domain]` on the table `Store` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `Store` ADD COLUMN `shopify_access_token` VARCHAR(191) NULL,
...
-- CreateIndex
CREATE UNIQUE INDEX `Store_shopify_shop_domain_key` ON `Store`(`shopify_shop_domain`);
