/*
  ...
  - You are about to drop the column `order_cart_store_code` on the `Ec_Message_Center` table. All the data in the column will be lost.

*/
-- DropForeignKey
ALTER TABLE `Ec_Message_Center` DROP FOREIGN KEY `Ec_Message_Center_order_cart_store_code_fkey`;

-- DropIndex
DROP INDEX `Ec_Message_Center_order_cart_store_code_fkey` ON `Ec_Message_Center`;
...
-- AlterTable
ALTER TABLE `Ec_Message_Center` DROP COLUMN `order_cart_store_code`,
    ADD COLUMN `order_id` INTEGER UNSIGNED NULL,
    ADD COLUMN `store_id` INTEGER UNSIGNED NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();
