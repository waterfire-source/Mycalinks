/*
  ...
  - A unique constraint covering the columns `[store_id,shopify_product_variant_id]` on the table `Product` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `Outbox_Ec_Product_Stock_History` ADD COLUMN `shopify_product_variant_id` BIGINT NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP();

-- AlterTable
ALTER TABLE `Outbox_Product` ADD COLUMN `shopify_product_variant_id` BIGINT NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP();
...
-- AlterTable
ALTER TABLE `Product` ADD COLUMN `shopify_product_variant_id` BIGINT NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
...
-- CreateIndex
CREATE UNIQUE INDEX `Product_store_id_shopify_product_variant_id_key` ON `Product`(`store_id`, `shopify_product_variant_id`);

-- 対象IDを一時テーブルに抽出
CREATE TEMPORARY TABLE ItemIDs AS
SELECT id FROM Item WHERE infinite_stock = 1;

-- それを使ってUPDATE
UPDATE Product
SET stock_number = 1000000
WHERE item_id IN (SELECT id FROM ItemIDs);

-- 一時テーブル削除（不要なら省略）
DROP TEMPORARY TABLE IF EXISTS ItemIDs;