/*
  ...
  - Made the column `total_discount_price` on table `Transaction` required. This step will fail if there are existing NULL values in that column.
  - Made the column `total_sale_price` on table `Transaction` required. This step will fail if there are existing NULL values in that column.
  - Made the column `total_discount_price` on table `Transaction_Cart` required. This step will fail if there are existing NULL values in that column.
  - Made the column `total_unit_price` on table `Transaction_Cart` required. This step will fail if there are existing NULL values in that column.
  - Made the column `total_sale_unit_price` on table `Transaction_Cart` required. This step will fail if there are existing NULL values in that column.
...
ALTER TABLE `Transaction` MODIFY `is_return` TINYINT UNSIGNED NOT NULL DEFAULT false,
    MODIFY `total_discount_price` INTEGER NOT NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
    ...
    MODIFY `total_sale_price` INTEGER NOT NULL;

-- AlterTable
ALTER TABLE `Transaction_Cart` MODIFY `total_discount_price` INTEGER NOT NULL,
    MODIFY `total_unit_price` INTEGER NOT NULL,
    MODIFY `total_sale_unit_price` INTEGER NOT NULL;
...
-- CreateIndex
CREATE INDEX `Transaction_Customer_Cart_transaction_id_idx` ON `Transaction_Customer_Cart`(`transaction_id`);

-- RedefineIndex
CREATE INDEX `Transaction_Customer_Cart_product_id_idx` ON `Transaction_Customer_Cart`(`product_id`);