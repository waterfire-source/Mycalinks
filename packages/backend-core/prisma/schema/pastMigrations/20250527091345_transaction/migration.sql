...
-- AlterTable
ALTER TABLE `Transaction` ADD COLUMN `set_deal_discount_price` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `total_reservation_price` INTEGER NOT NULL DEFAULT 0;

UPDATE Transaction SET set_deal_discount_price = total_discount_price - discount_price - point_discount_price;

-- AlterTable
ALTER TABLE `Transaction`
    ADD COLUMN `total_sale_price` INTEGER GENERATED ALWAYS AS (total_price - total_reservation_price) STORED,
    MODIFY `total_discount_price` INTEGER GENERATED ALWAYS AS (discount_price + set_deal_discount_price + point_discount_price) STORED,
...
-- AlterTable
ALTER TABLE `Transaction_Cart` ADD COLUMN `reservation_price` INTEGER NOT NULL DEFAULT 0;

ALTER TABLE `Transaction_Cart`
    MODIFY `total_discount_price` INTEGER GENERATED ALWAYS AS (discount_price + sale_discount_price) STORED,
    MODIFY `total_unit_price` INTEGER GENERATED ALWAYS AS (unit_price + reservation_price + total_discount_price) STORED,
    ADD COLUMN `total_sale_unit_price` INTEGER GENERATED ALWAYS AS (unit_price + total_discount_price) STORED;

-- AlterTable
ALTER TABLE `Transaction_Customer_Cart` ADD COLUMN `original_item_count` INTEGER NULL;
UPDATE Transaction_Customer_Cart SET original_item_count = item_count;


-- 権限グループ
UPDATE `Account_Group` SET `list_original_pack` = true, list_purchase_table = true WHERE `id` IN (100,101);