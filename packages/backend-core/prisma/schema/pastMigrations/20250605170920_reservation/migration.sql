/*
  ...
  - The values [ONGOING] on the enum `Reservation_status` will be removed. If these variants are still used in the database, this will fail.
  - The primary key for the `Reservation_Reception_Product` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - You are about to drop the column `deposit_item_count` on the `Reservation_Reception_Product` table. All the data in the column will be lost.
  - You are about to drop the column `received_item_count` on the `Reservation_Reception_Product` table. All the data in the column will be lost.
  - A unique constraint covering the columns `[reservation_reception_id,reservation_id]` on the table `Reservation_Reception_Product` will be added. If there are existing duplicate values, this will fail.
  - Added the required column `id` to the `Reservation_Reception_Product` table without a default value. This is not possible if the table is not empty.
...
ALTER TABLE `Reservation` MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
    MODIFY `status` ENUM('NOT_STARTED', 'OPEN', 'CLOSED', 'FINISHED', 'DELETED') NOT NULL DEFAULT 'NOT_STARTED';

-- AlterTable
ALTER TABLE `Reservation_Reception` MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP();

-- AlterTable
ALTER TABLE `Reservation_Reception_Product` DROP PRIMARY KEY,
    DROP COLUMN `deposit_item_count`,
    DROP COLUMN `received_item_count`,
    ADD COLUMN `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
    ADD COLUMN `status` ENUM('CREATED', 'DEPOSITED', 'RECEIVED', 'CANCELLED') NOT NULL DEFAULT 'DEPOSITED',
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
    ADD PRIMARY KEY (`id`);
...
-- AlterTable
ALTER TABLE `Transaction` ADD COLUMN `hidden` TINYINT UNSIGNED NOT NULL DEFAULT false,
...
-- CreateIndex
CREATE UNIQUE INDEX `Reservation_Reception_Product_reservation_reception_id_reser_key` ON `Reservation_Reception_Product`(`reservation_reception_id`, `reservation_id`);

-- CreateIndex
CREATE INDEX `System_Log_resource_id_resource_kind_idx` ON `System_Log`(`resource_id`, `resource_kind`);
