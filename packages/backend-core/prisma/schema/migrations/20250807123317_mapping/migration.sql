...
-- CreateTable
CREATE TABLE `Consignment_Client_Mapping` (
    `from_store_id` INTEGER UNSIGNED NOT NULL,
    `to_store_id` INTEGER UNSIGNED NOT NULL,
    `from_client_id` INTEGER UNSIGNED NOT NULL,
    `to_client_id` INTEGER UNSIGNED NOT NULL,

    PRIMARY KEY (`from_client_id`, `to_client_id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- AddForeignKey
ALTER TABLE `Consignment_Client_Mapping` ADD CONSTRAINT `Consignment_Client_Mapping_from_store_id_to_store_id_fkey` FOREIGN KEY (`from_store_id`, `to_store_id`) REFERENCES `Store_Relation`(`from_store_id`, `to_store_id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Consignment_Client_Mapping` ADD CONSTRAINT `Consignment_Client_Mapping_from_client_id_fkey` FOREIGN KEY (`from_client_id`) REFERENCES `Consignment_Client`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Consignment_Client_Mapping` ADD CONSTRAINT `Consignment_Client_Mapping_to_client_id_fkey` FOREIGN KEY (`to_client_id`) REFERENCES `Consignment_Client`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
