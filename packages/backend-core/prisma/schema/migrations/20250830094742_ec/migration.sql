...
-- AlterTable
ALTER TABLE `Ec_Order_Cart_Store` ADD COLUMN `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    ADD COLUMN `finished_at` TIMESTAMP(0) NULL,
...
-- AlterTable
ALTER TABLE `Ec_Order_Cart_Store_Product` ADD COLUMN `wholesale_total_price` INTEGER NULL;
...
-- AlterTable
ALTER TABLE `Ec_Setting` ADD COLUMN `commission_rate` INTEGER NOT NULL DEFAULT 11,
    MODIFY `auto_listing` TINYINT UNSIGNED NOT NULL DEFAULT false,
...
-- CreateTable
CREATE TABLE `Fact_Ec_Order_Product` (
    ...
    `order_created_at` TIMESTAMP(0) NOT NULL,
    `order_completed_at` TIMESTAMP(0) NOT NULL,
    `target_day` DATE NOT NULL,

    INDEX `Fact_Ec_Order_Product_order_id_product_id_idx`(`order_id`, `product_id`),
    INDEX `Fact_Ec_Order_Product_condition_option_id_idx`(`condition_option_id`),
    INDEX `Fact_Ec_Order_Product_condition_option_display_name_idx`(`condition_option_display_name`),
    INDEX `Fact_Ec_Order_Product_condition_option_handle_idx`(`condition_option_handle`),
    INDEX `Fact_Ec_Order_Product_category_id_idx`(`category_id`),
    INDEX `Fact_Ec_Order_Product_category_display_name_idx`(`category_display_name`),
    INDEX `Fact_Ec_Order_Product_category_handle_idx`(`category_handle`),
    INDEX `Fact_Ec_Order_Product_genre_id_idx`(`genre_id`),
    INDEX `Fact_Ec_Order_Product_genre_display_name_idx`(`genre_display_name`),
    INDEX `Fact_Ec_Order_Product_genre_handle_idx`(`genre_handle`),
    INDEX `Fact_Ec_Order_Product_specialty_id_idx`(`specialty_id`),
    INDEX `Fact_Ec_Order_Product_specialty_display_name_idx`(`specialty_display_name`),
    INDEX `Fact_Ec_Order_Product_store_id_idx`(`store_id`),
    PRIMARY KEY (`store_id`, `order_id`, `product_id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `Summary_Daily_Ec_Order` (
    `store_id` INTEGER UNSIGNED NOT NULL,
    `target_day` DATE NOT NULL,
    ...
    `cancel_price` INTEGER NOT NULL,
    `change_price` INTEGER NOT NULL,
    `item_count` INTEGER NOT NULL,
    `wholesale_price` INTEGER NOT NULL,

    INDEX `Summary_Daily_Ec_Order_store_id_idx`(`store_id`),
    PRIMARY KEY (`store_id`, `target_day`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateIndex
CREATE INDEX `Ec_Order_Cart_Store_store_id_finished_at_idx` ON `Ec_Order_Cart_Store`(`store_id`, `finished_at`);


UPDATE Ec_Order_Cart_Store
INNER JOIN Ec_Order ON Ec_Order_Cart_Store.order_id = Ec_Order.id
SET Ec_Order_Cart_Store.finished_at = Ec_Order.updated_at
WHERE Ec_Order_Cart_Store.status IN ('COMPLETED', 'CANCELED');