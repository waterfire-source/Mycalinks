/*
  ...
  - A unique constraint covering the columns `[store_id,shopify_product_id]` on the table `Product` will be added. If there are existing duplicate values, this will fail.
  - A unique constraint covering the columns `[store_id,shopify_inventory_item_id]` on the table `Product` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `Outbox_Ec_Product_Stock_History` ADD COLUMN `shopify_inventory_item_id` VARCHAR(50) NULL,
    ADD COLUMN `shopify_product_id` VARCHAR(50) NULL,
    MODIFY `shopify_product_variant_id` VARCHAR(50) NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP();

-- AlterTable
ALTER TABLE `Outbox_Product` ADD COLUMN `shopify_product_id` VARCHAR(50) NULL,
    MODIFY `shopify_product_variant_id` VARCHAR(50) NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP();
...
-- AlterTable
ALTER TABLE `Product` ADD COLUMN `shopify_inventory_item_id` VARCHAR(50) NULL,
    ADD COLUMN `shopify_product_id` VARCHAR(50) NULL,
...
-- CreateIndex
CREATE UNIQUE INDEX `Product_store_id_shopify_product_id_key` ON `Product`(`store_id`, `shopify_product_id`);

-- CreateIndex
CREATE UNIQUE INDEX `Product_store_id_shopify_inventory_item_id_key` ON `Product`(`store_id`, `shopify_inventory_item_id`);
