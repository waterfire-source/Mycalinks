...
-- AlterTable
ALTER TABLE `Ec_Order` ADD COLUMN `display_shipping_address` VARCHAR(100) NULL,
    ADD COLUMN `wish_list_id` INTEGER UNSIGNED NULL,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    MODIFY `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP();

-- AlterTable
ALTER TABLE `Ec_Order_Cart_Store` ADD COLUMN `wish_list_shipping_fee` INTEGER NULL,
...
-- AlterTable
ALTER TABLE `Ec_Order_Cart_Store_Product` ADD COLUMN `wish_list_product_id` INTEGER UNSIGNED NULL;
...
-- AlterTable
ALTER TABLE `Register_Settlement` ADD COLUMN `transaction_buy_discount_total` INTEGER NOT NULL DEFAULT 0,
    ADD COLUMN `transaction_sell_discount_total` INTEGER NOT NULL DEFAULT 0,
    MODIFY `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
...
-- AlterTable
ALTER TABLE `Summary_Daily_Transaction` ADD COLUMN `coupon_discount_price` INTEGER NOT NULL DEFAULT 0;
...
-- AlterTable
ALTER TABLE `Transaction` ADD COLUMN `coupon_discount_price` INTEGER NOT NULL DEFAULT 0,
    ...
    MODIFY `total_discount_price` INTEGER GENERATED ALWAYS AS (discount_price + set_deal_discount_price + point_discount_price + coupon_discount_price) STORED;

-- CreateTable
CREATE TABLE `Ec_Wish_List` (
    `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
    `title` VARCHAR(191) NOT NULL,
    `wisher_display_name` VARCHAR(191) NOT NULL,
    `myca_user_id` INTEGER NULL,
    `area_public` TINYINT UNSIGNED NOT NULL DEFAULT false,
    `shipping_address_public` TINYINT UNSIGNED NOT NULL DEFAULT false,
    `allow_other_products` TINYINT UNSIGNED NOT NULL DEFAULT false,
    `bought_count_delay` TINYINT UNSIGNED NOT NULL DEFAULT false,
    `public_description` VARCHAR(191) NULL,
    `status` ENUM('PRIVATE', 'PUBLIC', 'DELETED') NOT NULL DEFAULT 'PRIVATE',
    `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),

    INDEX `Ec_Wish_List_myca_user_id_idx`(`myca_user_id`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `Ec_Wish_List_Product` (
    `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
    `wish_list_id` INTEGER UNSIGNED NOT NULL,
    `myca_item_id` INTEGER NOT NULL,
    `condition_option_handle` ENUM('O1_S', 'O2_A', 'O3_FOR_PLAY', 'O4_B', 'O5_C', 'O6_D', 'O1_BRAND_NEW', 'O2_LIKE_NEW', 'O3_USED') NULL,
    ...
    `created_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP(),
    `updated_at` TIMESTAMP(0) NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
    `last_bought_at` TIMESTAMP(0) NULL,

    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- AddForeignKey
ALTER TABLE `Ec_Order` ADD CONSTRAINT `Ec_Order_wish_list_id_fkey` FOREIGN KEY (`wish_list_id`) REFERENCES `Ec_Wish_List`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Ec_Order_Cart_Store_Product` ADD CONSTRAINT `Ec_Order_Cart_Store_Product_wish_list_product_id_fkey` FOREIGN KEY (`wish_list_product_id`) REFERENCES `Ec_Wish_List_Product`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Ec_Wish_List_Product` ADD CONSTRAINT `Ec_Wish_List_Product_wish_list_id_fkey` FOREIGN KEY (`wish_list_id`) REFERENCES `Ec_Wish_List`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Ec_Wish_List_Product` ADD CONSTRAINT `Ec_Wish_List_Product_myca_item_id_fkey` FOREIGN KEY (`myca_item_id`) REFERENCES `Myca_Item`(`myca_item_id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `Ec_Wish_List_Product` ADD CONSTRAINT `Ec_Wish_List_Product_store_id_fkey` FOREIGN KEY (`store_id`) REFERENCES `Store`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
