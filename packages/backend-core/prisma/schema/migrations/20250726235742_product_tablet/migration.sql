/*
  ...
  - A unique constraint covering the columns `[store_id,item_category_id,item_genre_id,condition_option_id,specialty_id]` on the table `Tablet_Allowed_Genre_Category` will be added. If there are existing duplicate values, this will fail.

*/
-- DropForeignKey
ALTER TABLE `Tablet_Allowed_Genre_Category` DROP FOREIGN KEY `Tablet_Allowed_Genre_Category_store_id_fkey`;

-- DropIndex
DROP INDEX `Tablet_Allowed_Genre_Category_store_id_item_category_id_item_key` ON `Tablet_Allowed_Genre_Category`;
...
-- AlterTable
ALTER TABLE `Tablet_Allowed_Genre_Category` ADD COLUMN `limit_count` INTEGER UNSIGNED NOT NULL DEFAULT 0,
    ADD COLUMN `no_specialty` TINYINT UNSIGNED NOT NULL DEFAULT false,
...
-- CreateIndex
CREATE UNIQUE INDEX `Tablet_Allowed_Genre_Category_store_id_item_category_id_item_key` ON `Tablet_Allowed_Genre_Category`(`store_id`, `item_category_id`, `item_genre_id`, `condition_option_id`, `specialty_id`);

-- AddForeignKey
ALTER TABLE `Tablet_Allowed_Genre_Category` ADD CONSTRAINT `Tablet_Allowed_Genre_Category_specialty_id_fkey` FOREIGN KEY (`specialty_id`) REFERENCES `Specialty`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

UPDATE Item SET release_at = NULL
WHERE
    release_at IS NOT NULL AND 
    (release_at = '0000-00-00 00:00:00'
    OR release_at < '1900-01-01 00:00:00'
    OR release_at > NOW() + INTERVAL 10 YEAR);