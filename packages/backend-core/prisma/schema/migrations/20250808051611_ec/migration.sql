-- DropForeignKey
ALTER TABLE `Ec_Setting` DROP FOREIGN KEY `Ec_Setting_store_id_fkey`;
...
-- DropIndex
DROP INDEX `Ec_Setting_store_id_key` ON `Ec_Setting`;
...
-- CreateTable
CREATE TABLE `Ec_About_Us` (
    `store_id` INTEGER UNSIGNED NOT NULL,
    `shop_pr` TEXT NULL,
    `images` JSON NOT NULL,
    `about_shipping` TEXT NULL,
    `about_shipping_fee` TEXT NULL,
...
-- AddForeignKey
ALTER TABLE `Ec_About_Us` ADD CONSTRAINT `Ec_About_Us_store_id_fkey` FOREIGN KEY (`store_id`) REFERENCES `Store`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;