...
-- AlterTable
ALTER TABLE `Fact_Transaction_Product` ADD COLUMN `is_return` BOOLEAN NOT NULL DEFAULT false;
...
-- TransactionのFactレコードにis_returnを保持させる
UPDATE Fact_Transaction_Product
INNER JOIN Transaction ON Fact_Transaction_Product.transaction_id = Transaction.id
SET Fact_Transaction_Product.is_return = Transaction.is_return
WHERE Fact_Transaction_Product.is_return = true;
