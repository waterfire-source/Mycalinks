...
-- AlterTable
ALTER TABLE `Item` ADD COLUMN `release_at` TIMESTAMP(0) NULL,
...
SET release_at = CASE
    WHEN Item_Genre.handle = 'ポケモン' THEN
        STR_TO_DATE(CONCAT(Item.release_date, ' 07:00:00'), '%Y-%m-%d %H:%i:%s')
    ELSE
        STR_TO_DATE(CONCAT(Item.release_date, ' 00:00:00'), '%Y-%m-%d %H:%i:%s')
END
WHERE Item.release_date IS NOT NULL
