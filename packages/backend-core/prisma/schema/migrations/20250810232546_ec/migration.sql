/*
  ...
  - You are about to alter the column `shopify_shop_domain` on the `Ec_Setting` table. The data in that column could be lost. The data in that column will be cast from `VarChar(191)` to `VarChar(50)`.
  - A unique constraint covering the columns `[store_id,handle]` on the table `Specialty` will be added. If there are existing duplicate values, this will fail.
...
-- AlterTable
ALTER TABLE `Ec_Setting` ADD COLUMN `shopify_location_id` VARCHAR(50) NULL,
    ...
    MODIFY `shopify_shop_domain` VARCHAR(50) NULL;
...
-- CreateIndex
CREATE UNIQUE INDEX `Specialty_store_id_handle_key` ON `Specialty`(`store_id`, `handle`);
